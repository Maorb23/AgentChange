...
def test_fixture_based_sample_generation(tmp_path):
    ...
    assert any(event["command"] == "python -m ruff check ." and event["result_status"] == "succeeded" for event in serialized)
    assert any(event["command"] == "python -m pytest -q" and event["result_status"] == "failed" for event in serialized)
    stop = serialized[-1]
